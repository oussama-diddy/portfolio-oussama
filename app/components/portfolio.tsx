...
import projectNav from "./project-nav.module.css";
...
export default function Portfolio({ year }: { year: number }) {
  ...
  return <>
    ...
    <header ref={navbar} className={`navbar ${projectNav.navbar}`}><div className="container nav-inner"><a href="#home" className="logo" aria-label="Oussama home">OUSSAMA<span>®</span></a><nav id="main-navigation" className={menuOpen ? "nav-links open" : "nav-links"} aria-label="Main navigation">{["Home", "About", "Services", "Work", "Contact"].map(item => <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)}>{item}</a>)}<Link href="/start-a-project" onClick={() => setMenuOpen(false)}>Start a Project</Link></nav><a href="#contact" className="nav-cta">Let’s talk <Icon/></a><button className="menu-toggle" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} aria-controls="main-navigation" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? "✕" : "☰"}</button></div></header>
    <main id="main">
      <SectionTransition><section id="home" className="hero container"><div className="hero-copy"><div className="availability"><span/> Available for freelance projects</div><p className="hero-kicker">VIDEO EDITOR & VISUAL STORYTELLER</p><h1>Stories Cut<br/>With <span>Purpose.</span></h1><p className="hero-description">Freelance Video Editor focused on cinematic storytelling, social media content, ads and high-retention edits.</p><div className="button-row"><a className="button button-light" href="#work">View My Work <Icon/></a><a className="button button-outline" href="/start-a-project">Start a Project <Icon className="diagonal"/></a></div><div className="hero-signoff"><span className="tiny-line"/> GOOD STORIES DESERVE GREAT EDITS.</div></div><EditingVisual/><div className="hero-bottom"><span>FROM RAW FOOTAGE TO SOMETHING REMARKABLE.</span><a href="#about">SCROLL TO EXPLORE <span>↓</span></a></div></section></SectionTransition>
      ...
      <SectionTransition><section id="contact" className="contact-section container reveal"><div className="availability"><span/> OPEN FOR COLLABORATION</div><h2>Have a project<br/>in <span>mind?</span></h2><p>Let&apos;s create something people actually want to watch.</p><div className="button-row contact-social-buttons"><Link className="button button-outline" href="/start-a-project">Start a Project <Icon/></Link><a className="button button-light" href={contactLinks.whatsapp} target="_blank" rel="noopener noreferrer"><SocialIcon name="whatsapp"/> WhatsApp</a><a className="button button-outline" href={contactLinks.instagram} target="_blank" rel="noopener noreferrer"><SocialIcon name="instagram"/> Instagram</a></div><div className="contact-details"><a href={contactLinks.whatsapp} target="_blank" rel="noopener noreferrer">+212 705 532 747</a><a href={contactLinks.instagram} target="_blank" rel="noopener noreferrer">@oussama_sn17</a></div></section></SectionTransition>
    </main>
